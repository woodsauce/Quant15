const { json, missingEnv, coinbaseRequest } = require('./_coinbaseKeyAuth');
...
module.exports = async function handler(req, res) {
  ...
  if (missing.length) return json(res, 400, { ok: false, error: 'Missing Coinbase API key environment variables', missing, paperEquivalent: simulated });

  if (process.env.COINBASE_LIVE_TRADING_ENABLED !== 'true') {
    return json(res, 403, { ok: false, error: 'Live Coinbase trading is locked. Set COINBASE_LIVE_TRADING_ENABLED=true only after paper/key testing.', paperEquivalent: simulated });
  }

  try {
    const data = await coinbaseRequest('/api/v3/brokerage/orders', { method: 'POST', body: JSON.stringify(payload) });
    json(res, 200, { ok: true, mode: 'live', submitted: true, product_id: productId, side: 'BUY', quote_size: quoteSize, response: data });
  } catch (err) {
    json(res, err.status || 502, { ok: false, error: 'Coinbase live order failed', detail: err.message, response: err.data || null, payloadPreview: { ...payload, order_configuration: payload.order_configuration } });
  }
};
