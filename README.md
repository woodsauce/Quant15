...
## Vercel unmatched function pattern fix

This package removes the explicit `functions` glob from `vercel.json`. Vercel will auto-detect files in the root `api/` folder as serverless functions.

Make sure these files are uploaded/committed at the root of the project:

- `api/kalshi.js`
- `api/kalshi-market.js`
- `api/coinbase.js`
- `api/candles.js`
- `api/health.js`

If Vercel is connected to GitHub, replacing the ZIP locally is not enough. Commit these files to the GitHub repo that Vercel is cloning.
