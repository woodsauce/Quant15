...
const COINBASE_KEY = 'edge15-coinbase-oauth-spot-v1';
...
function init() {
  ...
  els.coinbaseDisconnectBtn?.addEventListener('click', disconnectCoinbase);
...
function afterLockCreated(lock) {
  // Coinbase OAuth spot mode is intentionally conservative: only OVER locks can become spot buys.
...
async function testCoinbaseConnection(show = true) {
  ...
  if (show) {
    els.coinbaseStatusText.textContent = 'Testing Coinbase OAuth...';
    els.coinbaseStatusPill.textContent = 'Testing...';
    els.coinbaseStatusPill.className = 'coinbase-status-pill';
  }
  try {
    const data = await fetchJsonNoAbort('/api/coinbase-oauth-status');
    state.coinbase.status = data;
    save(COINBASE_KEY, state.coinbase);
    if (data.connected) {
      els.coinbaseStatusPill.textContent = data.liveTradingEnabled ? 'Connected • Live server unlocked' : 'Connected • Live server locked';
      els.coinbaseStatusPill.className = 'coinbase-status-pill connected';
      els.coinbaseStatusText.textContent = data.authTest === false ? 'Connected, but account test failed' : `Connected to Coinbase (${data.accountsCount ?? 0} accounts visible)`;
    } else {
      ...
      els.coinbaseStatusText.textContent = `Coinbase not connected.${missing}`;
    }
    renderCoinbase();
  } catch (err) {
...
async function disconnectCoinbase() {
  if (!confirm('Disconnect Coinbase OAuth from this browser session?')) return;
  try {
    await fetchJsonNoAbort('/api/coinbase-oauth-disconnect', { method: 'POST' });
  } catch {}
  state.coinbase.status = { connected: false };
