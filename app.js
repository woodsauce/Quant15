...
const COINBASE_KEY = 'edge15-coinbase-simple-key-trader-v1';
...
const state = {
  ...
  coinbase: load(COINBASE_KEY, { mode: 'off', usdSize: 1, events: [] }),
  loading: false,
  lastRoundKey: '',
  abort: null
};
...
const els = {
  ...
  coinbaseMode: document.getElementById('coinbaseMode'),
  coinbaseUsdSize: document.getElementById('coinbaseUsdSize'),
  coinbaseStatusBtn: document.getElementById('coinbaseStatusBtn'),
  coinbaseDisconnectBtn: document.getElementById('coinbaseDisconnectBtn'),
  coinbasePrepareEnvBtn: document.getElementById('coinbasePrepareEnvBtn'),
  coinbaseClearSetupBtn: document.getElementById('coinbaseClearSetupBtn'),
  coinbaseKeyNameInput: document.getElementById('coinbaseKeyNameInput'),
  coinbasePrivateKeyInput: document.getElementById('coinbasePrivateKeyInput'),
  coinbaseEnvOutput: document.getElementById('coinbaseEnvOutput'),
  coinbaseStatusPill: document.getElementById('coinbaseStatusPill'),
  coinbaseStatusText: document.getElementById('coinbaseStatusText'),
  coinbaseTradeText: document.getElementById('coinbaseTradeText'),
  coinbaseLog: document.getElementById('coinbaseLog')
};
...
function init() {
  ...
  if (els.coinbaseMode) {
    els.coinbaseMode.value = state.coinbase.mode || 'off';
    els.coinbaseMode.addEventListener('change', () => { state.coinbase.mode = els.coinbaseMode.value; save(COINBASE_KEY, state.coinbase); renderCoinbase(); });
  }
  if (els.coinbaseUsdSize) {
    els.coinbaseUsdSize.value = state.coinbase.usdSize || 1;
    els.coinbaseUsdSize.addEventListener('change', () => { state.coinbase.usdSize = clamp(Number(els.coinbaseUsdSize.value), 1, 1000); els.coinbaseUsdSize.value = state.coinbase.usdSize; save(COINBASE_KEY, state.coinbase); });
  }
  els.coinbaseStatusBtn?.addEventListener('click', testCoinbaseConnection);
  els.coinbaseDisconnectBtn?.addEventListener('click', clearCoinbaseLocalLog);
  els.coinbasePrepareEnvBtn?.addEventListener('click', prepareCoinbaseEnvValues);
  els.coinbaseClearSetupBtn?.addEventListener('click', clearCoinbaseSetupHelper);
...
function exportRecord() {
  const payload = {
    ...
    coinbaseSpotTrader: { mode: state.coinbase.mode, usdSize: state.coinbase.usdSize, events: state.coinbase.events },
...
function afterLockCreated(lock) {
  // Coinbase Simple Key spot mode is intentionally conservative: only OVER locks can become spot buys.
  // UNDER locks remain signal-only unless you later add shorting or sell-owned-asset logic.
  if (!state.coinbase || state.coinbase.mode === 'off') return;
  sendCoinbaseSignal(lock);
}

async function testCoinbaseConnection(show = true) {
  if (!els.coinbaseStatusText) return;
  if (show) {
    els.coinbaseStatusText.textContent = 'Testing Coinbase API key...';
    els.coinbaseStatusPill.textContent = 'Testing...';
    els.coinbaseStatusPill.className = 'coinbase-status-pill';
  }
  try {
    const data = await fetchJsonNoAbort('/api/coinbase-key-status');
    state.coinbase.status = data;
    save(COINBASE_KEY, state.coinbase);
    if (data.connected && data.authTest) {
      els.coinbaseStatusPill.textContent = data.liveTradingEnabled ? 'Connected • Live server unlocked' : 'Connected • Paper/live locked';
      els.coinbaseStatusPill.className = 'coinbase-status-pill connected';
      const trade = data.canTrade === true ? 'trade permission detected' : 'trade permission not detected';
      els.coinbaseStatusText.textContent = `Coinbase key works (${trade})`;
    } else {
      els.coinbaseStatusPill.textContent = 'Not connected';
      els.coinbaseStatusPill.className = data.jwtCreated ? 'coinbase-status-pill error' : 'coinbase-status-pill';
      const missing = Array.isArray(data.missingEnv) && data.missingEnv.length ? ` Missing env: ${data.missingEnv.join(', ')}` : '';
      els.coinbaseStatusText.textContent = `${data.error || 'Coinbase key not connected.'}${missing}`;
    }
    renderCoinbase();
  } catch (err) {
    els.coinbaseStatusPill.textContent = 'Coinbase error';
    els.coinbaseStatusPill.className = 'coinbase-status-pill error';
    els.coinbaseStatusText.textContent = err.message || 'Coinbase status failed';
  }
}

function clearCoinbaseLocalLog() {
  if (!confirm('Clear local Coinbase event log and turn Coinbase mode off?')) return;
  state.coinbase.events = [];
  state.coinbase.status = state.coinbase.status || {};
  state.coinbase.mode = 'off';
  if (els.coinbaseMode) els.coinbaseMode.value = 'off';
  save(COINBASE_KEY, state.coinbase);
  renderCoinbase();
}
...
function prepareCoinbaseEnvValues() {
  const keyName = String(els.coinbaseKeyNameInput?.value || '').trim();
  const keyText = normalizePrivateKeyText(els.coinbasePrivateKeyInput?.value || '');
  if (!els.coinbaseEnvOutput) return;
  if (!keyName || !keyText) {
    els.coinbaseEnvOutput.textContent = 'Paste both the Coinbase API key name and private key text first.';
    return;
  }
  if (!/BEGIN .*PRIVATE KEY/.test(keyText)) {
    els.coinbaseEnvOutput.textContent = 'That does not look like a private key. It should start with -----BEGIN EC PRIVATE KEY----- or -----BEGIN PRIVATE KEY-----.';
    return;
  }
  try {
    const b64 = btoa(unescape(encodeURIComponent(keyText)));
    els.coinbaseEnvOutput.textContent = [
      'Add these in Vercel → Project → Settings → Environment Variables:',
      '',
      `COINBASE_API_KEY_NAME=${keyName}`,
      `COINBASE_PRIVATE_KEY_B64=${b64}`,
      'COINBASE_LIVE_TRADING_ENABLED=false',
      '',
      'Save them, redeploy, then click Test Coinbase Key in this dashboard.',
      'Do not paste these values into chat.'
    ].join('\n');
  } catch (err) {
    els.coinbaseEnvOutput.textContent = `Could not convert key to Base64: ${err.message}`;
  }
}

function clearCoinbaseSetupHelper() {
  if (els.coinbaseKeyNameInput) els.coinbaseKeyNameInput.value = '';
  if (els.coinbasePrivateKeyInput) els.coinbasePrivateKeyInput.value = '';
  if (els.coinbaseEnvOutput) els.coinbaseEnvOutput.textContent = 'Your Vercel values will appear here.';
}

async function sendCoinbaseSignal(lock) {
  const event = {
    id: `cb-${lock.id}`,
    ts: Date.now(),
    lockId: lock.id,
    ...
    symbol: lock.symbol,
    pick: lock.pick,
    mode: state.coinbase.mode,
    usdSize: state.coinbase.usdSize,
    status: 'submitting',
    message: 'Sending Coinbase spot signal...'
  };
  state.coinbase.events.unshift(event);
  state.coinbase.events = state.coinbase.events.slice(0, 25);
  save(COINBASE_KEY, state.coinbase);
  renderCoinbase();

  try {
    const data = await fetchJsonNoAbort('/api/coinbase-order', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ mode: state.coinbase.mode, usdSize: state.coinbase.usdSize, lock })
    });
    event.status = data.ok ? (data.skipped ? 'skipped' : data.submitted ? 'submitted' : 'paper') : 'error';
    event.message = data.skipped ? data.reason : data.submitted ? 'Coinbase live order submitted.' : data.paperOrder ? 'Paper Coinbase BUY logged. No real order was sent.' : (data.error || 'Coinbase response received.');
    event.response = data;
  } catch (err) {
    event.status = 'error';
    event.message = err.message || 'Coinbase signal failed.';
  }
  save(COINBASE_KEY, state.coinbase);
  renderCoinbase();
}
...
function renderCoinbase() {
  if (!els.coinbaseLog) return;
  if (els.coinbaseMode && els.coinbaseMode.value !== (state.coinbase.mode || 'off')) els.coinbaseMode.value = state.coinbase.mode || 'off';
  if (els.coinbaseUsdSize && Number(els.coinbaseUsdSize.value) !== Number(state.coinbase.usdSize || 1)) els.coinbaseUsdSize.value = state.coinbase.usdSize || 1;
  const events = state.coinbase.events || [];
  const modeText = state.coinbase.mode === 'live' ? 'LIVE requested' : state.coinbase.mode === 'paper' ? 'Paper mode' : 'Off';
  if (els.coinbaseTradeText) els.coinbaseTradeText.textContent = `${modeText} • OVER-only spot buys • $${state.coinbase.usdSize || 1} per OVER lock`;
  if (!events.length) {
    els.coinbaseLog.innerHTML = '<p class="small-note">Coinbase events will appear here after a lock.</p>';
    return;
  }
  els.coinbaseLog.innerHTML = events.slice(0, 8).map(ev => {
    const cls = ev.status === 'error' ? 'bad' : ev.status === 'skipped' ? 'warn' : 'good';
    return `<div class="coinbase-event ${cls}"><div><strong>${escapeHtml(ev.symbol)} ${escapeHtml(ev.pick)} • ${escapeHtml(ev.mode)}</strong><span>${escapeHtml(ev.message || ev.status)}</span></div><small>${new Date(ev.ts).toLocaleTimeString()}</small></div>`;
  }).join('');
}

async function fetchJsonNoAbort(url, options = {}) {
  const res = await fetch(url, { cache: 'no-store', ...options });
  const text = await res.text();
  let data = {};
  try { data = JSON.parse(text); } catch { data = { raw: text }; }
  if (!res.ok) throw new Error(data.detail || data.error || `${url} failed: ${res.status}`);
  return data;
}
